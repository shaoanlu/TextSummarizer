...
# Legacy (intented to show properties (size, # of object) of database)
"""
@app.route('/dbTS/')
def dbIndex():
    articles = []
    #temp = Comment.query.filter_by(content="contents002").first()
    temp_c = dbArticles.query.count()
    article = {'title':"Database content", 'img':'', 'text': '', 'keywords': ''}
    article['title'] = temp_c#temp.content
    article['liked_on'] = 1
    article['source_url'] = "http://salu.pythonanywhere.com/"
    articles.append(article)
    return render_template("dbTextSummarizer.html", articles=sorted(articles, key=lambda article: article["liked_on"], reverse=True))
"""
...
@app.route('/dbShowArticles/', methods=['POST'])
def dbArticlesDelete():
    # <form  method='POST' action="/dbArticlesDelete"> didn't work, always recieve GET
    # Guess: /dbShowArticles and /dbArticlesDelete are different, GET was sent first
    # ???: but <form  method='POST' action="/"> worked, why???
    # Delete by title is no a good idea: deleting "Gossip is a social skill - not a character flaw" failed (caused by ascii/unicode mismatch?)
    #    The original dash-sign caused ascii problem
    #    Sol: del. by id
    if request.method == 'POST':
        print "dbArticlesDelete() method: post"
        print request
        print request.form['delete']
        delId = request.form['delete']
        data = dbArticles.query.filter_by(id=delId).first()
        if not data is None:
            db.session.delete(data)
            db.session.commit()
        data = dbSummary.query.filter_by(id=delId).first()
        if not data is None:
            db.session.delete(data)
            db.session.commit()
    else:
...
@app.route('/dbFBSaved/')
def dbFBSaved():
    # Update dbFacebookSaved database through logging in to facebook and parsing its html
    # Input: None (email & pswd are hard coded)
    # Output: redirect to dbFBShow which shows updated article list in fb database
    urls = []
    articles = []
    # Using mechanize to log in to face book and get html of the 'saved' page
    text = mechanize_login_and_get_html()

    # Preprocessing html text, Clearing \\, // and replacing https to http
    urls = parse_fbsaved_html_to_urls(text)

    for url in urls:
        url = url.replace("\\","") # this line may be redundant
        story_url = url
        story_url = "http://" + story_url
        source_url = story_url
        print source_url

        find_duplicate = dbFacebookSaved.query.filter_by(link=source_url).first()
        if not find_duplicate:
            #if the url is valid, parse it (and adding 'archive.org/...' to the url), else continue to next url
            if True:#validators.url(story_url): # dont know why validators give False result
                story_url = parse_story_url(story_url)
                print "valid url"
            else:
                print "invalid url"
                continue
            # story_url: a json from archive.org api
            story_url = json.loads(story_url.text)

            #if there is a cache page in archive.org, extract the url from the json; else continue to next url
            if 'archived_snapshots' in story_url and story_url['archived_snapshots']:
                story_url = story_url['archived_snapshots'].get('closest',{}).get('url')
                print "cache exist"
            else:
                print "cache not exist"
                archive_org_save(source_url)
                continue

            # find duplicates by comparing source_url, if not duplicate, extrac_article() and add to dbFacebookSaved
            ...
            if not find_duplicate:
                ...
                if not article == None:
                    ...
                    articles.append(article)
                    timedate = strftime("%Y-%m-%d %H:%M:%S", gmtime())[0:11]
                    dbfbsaved = dbFacebookSaved(title = article['title'], link = article['source_url'], imglink = article['img'], summary = "[/////]".join(article['text']), keywords = article['keywords'], date = timedate)
                    db.session.add(dbfbsaved)
                    db.session.commit()
                    print "add to database"
                else:
                    print "article: None, not adding to db"
            else:# this block might be redundant
                print "duplicate url (in if statement)"
                continue
        else:
                print "duplicate url"
                continue
    return redirect(url_for('.dbFBShow'))
